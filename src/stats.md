...
间段。不同颜色的块显示你每天回答的卡片中有多少是[成熟](getting-started.md#卡片类型)、年轻、重新学习
或学习卡片。还有一个单独的组用于在过滤/死记硬背牌组中回答的卡片，这些卡片在当时未到期。线图和右轴则
显示随时间推移的每种类型的复习累积总数（所以在 0 天时会显示图表显示的整个时间段的数字）。

...
或者，你可以在浏览界面中使用[查找和替换](../browsing.md)功能一次性更新所有字段。如果每个字段都包含如
...
与集合中的现有笔记匹配，并且该笔记的模板与导入的模板相同，则现有笔记的其他字段将根据导入文件的内容进
行更新。导入界面中的下拉框允许你更改此行为，可以完全忽略重复，或将它们作为新笔记导入而非更新现有笔
记。
...
你必须在首选项界面中启用新的导入选项才能在 2.1.54 上使用此功能。在 2.1.55 上，新的导入路径是默认路
径。

...
默认样式适用于整张卡片。你还可以使某些字段或卡片的某部分使用不同的字体、颜色等。这在学习外语时尤其重
...
这会改变整张卡片的方向。你可以通过在一些 HTML 中包装字段参考来仅更改某些字段的方向：
...
之后，你可以选择更改整张卡片的字体，或仅更改某个字段的字体。要更改整张卡片的字体，只需在 .card 部分

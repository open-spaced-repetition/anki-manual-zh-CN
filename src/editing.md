...
「完形填空」指的是隐藏句子中的一个或多个词语的过程。例如，如果你这么一条句子：
...
Anki 提供了一种特殊的完形填空的笔记模板，以便于挖空。要创建一个完形填空笔记，选择填空笔记模板，并在
「文本」字段中输入一些文本。然后拖动鼠标选择要挖空的文本，并点击 \[…\] 按钮。Anki 将用以下内容替换文
本：
...
Anki 23.10+ 原生支持图片遮盖（Image Occlusion）卡片。图片遮盖笔记是基于图像的完形填空的特殊情况，允
许你创建卡片，隐藏图像的某些部分，测试你对隐藏信息的知识。

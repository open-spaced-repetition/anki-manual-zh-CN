...
窗口的左上角显示了当前的[笔记类型](getting-started.md#笔记类型)。如果不是「基础」，那么你可能在下载
共享牌组时添加了一些笔记类型。下面的文本假定「基础」已被选择。
...
注意：标记主要是为了兼容早期的 Anki 版本；大多数用户会更倾向于使用[标记](editing.md#使用旗标)。
...
「c1」部分表示你在这个句子上创建了一个完形填空。你可以创建多个删除，如果你愿意。例如，如果你选择堪培
拉并再次点击 \[…\]，则文本现在看起来像这样：
...
堪培拉和 1913。如果在创建完形填空时按住 <kbd>Alt</kbd> (<kbd>Option</kbd> 在 Mac 上)，Anki 将自动使
用相同的编号而不递增。

完形填空不需要在词界上进行，因此如果你选择「anberra」而不是「堪培拉」，问题将显示为「C\[…​\] 在 1913
成立」，给你一个提示。
...
Anki 23.10+ 原生支持图像遮掩卡片。图像遮掩（IO）笔记是基于图像的完形填空的特殊情况，允许你创建卡片，
...
要将 IO 卡片添加到你的集合中，打开添加界面，点击「类型」，从内置笔记类型列表中选择「图像遮掩」。然

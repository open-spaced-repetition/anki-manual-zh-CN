...
窗口的左上角显示了当前的[笔记模板](getting-started.md#笔记模板)。如果不是「基础」，那么你可能在下载
共享牌组时添加了一些笔记模板。下面的文本假定「基础」已被选择。
...
要创建一种新类型的笔记，请从主 Anki 窗口中选择工具 → 管理笔记模板。然后点击「添加」，添加一种新的笔
记类型。现在会看到另一个屏幕，它让你选择基于哪种笔记模板创建新的类型。「添加」是指基于 Anki 附带的一
...
- **默认使用 HTML 编辑器**很有用，如果你更喜欢直接在 HTML 中编辑字段。
...
有关联卡都会被加上标签。如果你需要为单个卡片加标签，而不加上其兄弟，你应该考虑使用旗标代替。

### 使用旗标

旗标类似于标签，但会在复习窗口中显示旗标的小图标，

出现在屏幕的右上角。你还可以在浏览界面中搜索被旗标标记的卡片、从浏览器中重命名旗标，以及从被旗标标记
的卡片创建筛选牌组，但不同于标签的是，一张卡片在同一时间只能有一个标记。另一个重要的区别是旗标作用
于[卡片](getting-started.md#卡片)级别，因此用旗标标记一张卡片不会影响其关联卡。
...
下、积压时等），那么从标签、标记、标记或字段创建的筛选牌组会更好。
...
「完形填空」的过程是隐藏句子中的一个或多个词语。例如，如果你有句子：

    堪培拉成立于 1913 年

……然后你在「1913 年」上创建一个完形填空，则句子将变为：

    堪培拉成立于 [...]

有时以这种方式挖空的部分被称为「遮蔽」。

为什么你可能想要使用完形填空的更多信息，请参见
[这里](https://super-memory.com/articles/20rules.htm)的规则 5。

Anki 提供了一种特殊的完形填空类型的笔记，以便于创建完形填空。要创建一个完形填空笔记，选择填空笔记类
型，并在「文本」字段中输入一些文本。然后拖动鼠标选择要隐藏的文本，并点击 \[…\] 按钮。Anki 将用以下内
容替换文本：

    堪培拉成立于 {{c1::1913 年}}

「c1」部分表示你在这个句子上创建了一个完形填空。你可以创建多个挖空，如果你愿意。例如，如果你选择堪培
拉并再次点击 \[…\]，则文本现在看起来像这样：
...
堪培拉和 1913 年。如果在创建完形填空时按住 <kbd>Alt</kbd> (<kbd>Option</kbd> 在 Mac 上)，Anki 将自动
使用相同的编号而不递增。

完形填空不需要在词界上进行，因此如果你选择「培拉」而不是「堪培拉」，问题将显示为「堪\[…​\]成立于
1913 年」，给你一个提示。
...
有关测试你正确输入完形填空答案的方法，请参见 [输入答案](templates/fields.md#检查你的答案)部分。

从版本 2.1.56 开始，支持嵌套完形填空。例如，以下是有效的：
...
在 2.1.56 版本之前，如果你需要从重叠文本创建完形填空，添加另一个文本字段到填空中，添加
...
填空笔记模板被 Anki 特殊处理，并不能基于普通笔记模板创建。如果你希望自定义它，请确保克隆现有的填空类
型，而不是其他类型的笔记。事物如格式可以自定义，但不能向填空笔记模板添加额外的卡片模板。

## 图像遮罩

Anki 23.10+ 原生支持图像遮罩卡片。图像遮罩（IO）笔记是基于图像的完形填空的特殊情况，允许你创建卡片，
...
要将图像遮罩卡片添加到你的集合中，打开添加界面，点击「类型」，从内置笔记模板列表中选择「图像遮罩」。
然后，点击「选择图像」以加载保存于电脑硬盘上的图像文件，或点击「从剪贴板粘贴图像」如果你已将图像复制
到剪贴板上。

### 添加图像遮罩卡片

加载图像后，图像遮罩编辑器将打开。点击左侧的图标，向图像添加所需数量的区域。有三个基本形状可供选择：

- 矩形
- 椭圆形
- 多边形

你还可以为每个笔记选择两种不同的图像遮罩模式：

- **隐藏所有，猜一个**：所有区域都被隐藏，同时仅显示一个区域进行学习。
- **隐藏一个，猜一个**：每次仅隐藏一个区域，并将在学习时显示。其他区域将可见。

![图像遮罩模式](media/io_modes.jpg)
...
你可以通过在复习时点击「编辑」或直接从浏览器中编辑你的图像遮罩笔记。这里有几个工具供你使用。值得注意
的有：
...
在复习图像遮罩卡片时，「切换遮罩」按钮会出现在图像正下方。当使用「隐藏所有，猜一个」模式时，这个按钮
会临时清除笔记的所有形状。

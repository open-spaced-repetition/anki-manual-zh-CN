...
控制学习重复的次数和它们之间的延迟。必须输入一个或多个用空格分隔的延迟。每次你在复习中按「良好」，卡
片就进入下一阶段。
...
隔一过就立即显示，优先于其他等待的卡片（如复习）。这样做是为了让你尽量在请求的延迟时回答卡片。相反，
如果间隔跨越日界线，则自动转换为天。

### 毕业间隔

在学习卡片没有初学间隔剩下时按下「良好」后，学卡片的延迟天数，以及作为复习卡片再次看到它的时间。这意
...
「简单」按钮立即将学习卡片变为复习卡片，并为其分配已配置的延迟。它应该始终至少与**毕业间隔**一样长，
通常还要多几天。
...
卡片后，后续复习中回答「良好」会将延迟增加大约 2.5 倍（例如，如果最后的延迟是 10 天，则下一个延迟将

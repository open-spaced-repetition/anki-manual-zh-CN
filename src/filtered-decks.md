...
见任务的便捷预设过滤，如复习当天失败的卡片。它会创建一个名为「自定义学习会话」的筛选牌组，并自动为你
打开。
...
高级用户可以使用任意搜索字符串（或「筛选器」）创建筛选牌组，而不是依赖预设筛选器。要手动创建过滤牌
组，请从工具菜单中选择「创建筛选牌组」。
...
在[v3 调度器](https://faqs.ankiweb.net/the-2021-scheduler.html)中，禁用重新排程的筛选牌组显示 4 个按
...
默认情况下，Anki 将通过在筛选牌组中的表现来调整计划后返回卡片到它们的首页牌组。有一个**基于我的答案
...
当你在复习中落后时，筛选牌组对于补课非常有用。一位 Anki 用户描述了他们使用筛选牌组来补课的方法如下：

我用过滤子牌组处理了 800 张卡片的积压。对我来说效果很好。

刚刚到期的筛选器为：「is:due prop:due>-7」

超过到期的筛选器为：「is:due prop:due<=-7」

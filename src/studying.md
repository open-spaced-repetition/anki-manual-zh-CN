...
- **困难**会重复当前步骤。
  ...
  - 如果卡片处在第一步（并且是唯一的一步），间隔是步骤的 50% 以上。但此间隔最多比步骤多一天。
  - 如果卡片处在第一步并且你设置了多于一步的步骤，间隔将是重来和良好的平均值，即第一、第二步的平均
    值。
  - 如果卡片处在后续步骤中，困难会重复之前的间隔。
...
- **简单**告诉 Anki 间隔太短。卡片安排会比「良好」更
  久，[并且 Anki 将在未来更少地安排该卡片](deck-options.md#简单复习间隔乘数)。因为「简单」迅速增加了
  间隔，最好只用于最简单的卡片。通常你应该发现自己在回答「良好」。
...
当只有问题展示时，Anki 在屏幕底部显示三个数字，如 6 + 9 + 59。它们代表未学习的卡片（蓝色）、学习中的
...
置了多个步骤，数字将在你遗忘一张卡片时增长超过 1，因为那张卡片需要被多次展示。
...
卡片放置在列表中较高的牌组中。当计算机按字母顺序对文本进行排序时，“-” 字符位于字母字符之前，“\~” 位
于之后。因此，你可以将牌组称为「-Vocab」以使其首先出现，并将另一牌组称为「\~My Textbook」以使其在所
有其他内容之后出现。
...
或 <kbd>Enter</kbd> 选择「良好」按钮。你可以使用 <kbd>1</kbd>-<kbd>4</kbd> 键选择特定的评分按钮。许
多人发现，用 <kbd>Space</kbd> 回答大多数卡片，然后将一根手指放在 <kbd>1</kbd> 上，以备遗忘时使用很方
便。

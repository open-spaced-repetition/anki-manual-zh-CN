...
能准确地比较你的答案，可以让 Anki [提示你输入答案](templates/fields.md#检查你的答案) 而不是仅仅显示
答案给你。
...
- **困难** 会重复当前步骤。
  ...
  - 如果卡片处在第一步并且你设置了多于一步的步骤，延迟将是重来和一般的平均值，即第一、第二步的平均
    值。
  - 如果卡片处在后续步骤中，困难会重复之前的延迟。

- **一般** 会将卡片移至[下一步](deck-options.md#学习步骤)。如果卡片处在最后一步，卡片将被转换为复习
  卡片（它完成『毕业』）。默认情况下，一旦卡片达到学习步骤的结束，卡片将在第二天再次展示，然后在越来
  越长的延迟后再次展示（见下一节）。

- **简单** 立即将卡片转换为复习卡片，即使还有步骤剩余。[默认](deck-options.md#简单间隔)情况下，卡片
  将在 4 天后再次展示，然后在越来越长的延迟后再次展示。在 v1 调度程序中，如果在重学模式下，则不会显
  示「简单」按钮，因为它会给出和「一般」一样的间隔。在
  [v2 调度程序+](https://faqs.ankiweb.net/the-anki-2.1-scheduler.html) 中，当卡片处于重学时，「简
  单」按钮会使间隔增加 1 天。

当卡片第一次出现时，它们从第一步开始。这意味着第一次在卡片上回答 **一般** 时，将在 10 分钟后再显示一
次，并且会跳过初始的 1 分钟步骤。但如果你按下 **重来**，卡片将在 1 分钟后回到。

你可以使用键盘上的 <kbd>1</kbd>, <kbd>2</kbd>, <kbd>3</kbd> 和 <kbd>4</kbd> 键来选择特定的按钮，其中
<kbd>1</kbd> 是 **重来**。按下 <kbd>Space</kbd> 或 <kbd>Enter</kbd> 将选择 **一般**。
...
- **重来** 将你的答案标记为不正确，并要求 Anki 在未来更频繁地展示该卡片。卡片被称为『失效』。请参
  见[lapse](deck-options.md#lapses) 部分以获取有关如何处理失效复习的更多信息。

- **困难** 默认情况下，比上次以稍长的延迟展示卡片，并要求 Anki 在未来更频繁地展示卡片。

- **一般** 告诉 Anki 上次的延迟大约合适，卡片难度不需要调低或调高。
  在[默认起始难度](deck-options.md#初始简易度)下，卡片将在上一次大约 2.5 倍的时间后再次展示，因此如
  果你上次等待了 10 天才看到该卡片，则下次的延迟约为 25 天。

- **简单** 告诉 Anki 延迟太短。卡片安排会比「一般」更
  久，[并且 Anki 将在未来更少地安排该卡片。](deck-options.md#「简单」复习间隔乘数) 因为『简单』迅速
  增加了延迟，最好只用于最简单的卡片。通常你应该发现自己在回答「一般」。

如同学习卡片一样，你可以使用键盘上的 <kbd>1</kbd>, <kbd>2</kbd>, <kbd>3</kbd> 和 <kbd>4</kbd> 来选择
答案。按下 <kbd>空格键</kbd> 或 <kbd>Enter</kbd> 将选择 **一般**。
...
- [**标记卡片**](editing.md#使用旗标)：为卡片添加一个彩色标记，或者关闭它。标记将在学习期间出现，你
  可以在浏览屏幕中搜索标记的卡片。当你想在稍后对卡片执行某些操作时，这很有用，比如回家后查单词。如果
  你使用的是 Anki 2.1.45+，也可以从[浏览器中](browsing.md)重命名标记。

- **搁置卡片 / 笔记**：隐藏卡片或该笔记的所有卡片直到第二天。（如果你想在之前取消搁置，可以点
  击[牌组概览](studying.md#学习概览)屏幕上的『取消搁置』按钮。）这在你目前无法回答卡片或想在其他时间
  重新考虑它们时很有用。搁置也可以[自动发生](studying.md关联卡与搁置)在同一笔记的卡片。

旧调度程序中，如果卡片处于学习中被搁置，它们会在被搁置前移动回到新卡或复习卡队列。

然而，使用[2.1 调度程序](https://faqs.ankiweb.net/the-anki-2.1-scheduler.html)，搁置卡片并不会重置卡
片的学习步骤。

- **遗忘卡片**：将当前卡片移至[新卡队列的末尾](browsing.md#卡片)。
...
- **设定到期日**：将卡片放入复习队列，并[使其在特定日期到期。](browsing.md#卡片)
...
- **上一个卡片信息**：显示关于上一张卡片的[统计信息](stats.md#卡片信息)。

- [**标记笔记**](editing.md#「标记」标签g)：为当前笔记添加“标记”标签，方便在浏览器中找到。这类似于标
  记单个卡片，但使用标签代替。因此，如果笔记有多张卡片，所有卡片在搜索标记标签时都会出现。大多数用户
  倾向于使用标记功能。
...
对于新卡片的更细粒度排序，你可以在[浏览器](browsing.md)中更改顺序。
...
当你回答一张有同类卡片的卡片时，Anki 可以通过自动“隐藏”它们防止这些卡片在同一个会话中显示。隐藏的卡
片在时钟转到新的一天之前或你手动通过「取消隐藏」按钮（可见于[牌组概览](studying.md#学习概览)屏幕底
部）取消隐藏之前将不会显示。即使同类卡片不在同一牌组中，Anki 也会隐藏它们（例如，如果你使用
了[覆盖牌组](templates/intro.md)功能）。

你可以在[牌组选项](deck-options.md)屏幕中启用隐藏 - 新卡片和复习卡片有独立的设置。

Anki 只会隐藏新卡片或复习卡片的同类卡片，不会隐藏正在学习中的卡片，因为对这些卡片来说，时间至关重
要。另一方面，当你学习一张正在学习中的卡片时，任何新的/复习中的同类卡片都会被隐藏。

请注意，从[v2 调度程序](https://faqs.ankiweb.net/the-anki-2.1-scheduler.html)开始，Anki 现在区分手动
隐藏和自动隐藏的卡片，以便你可以取消隐藏一组而不影响另一组。
